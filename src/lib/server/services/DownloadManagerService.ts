...
>() {
	static readonly Default = Layer.scoped(
		DownloadManager,
		Effect.gen(function* () {
			yield* Effect.logDebug('🧵 Initializing DownloadManager Service');
			...
			return DownloadManager.of({
				startDownloadFiber: Effect.fn('DownloadManager.addDownloadFiber')(function* (itemId, effect) {
					...
					yield* Effect.logDebug(`Adding fiber for itemId ${itemId}`);
				...
				cancelDownloadFiber: Effect.fn('DownloadManager.cancelDownloadFiber')(function* (itemId) {
					yield* Effect.logDebug(`Attempting to cancel fiber for itemId ${itemId}`);
					...
					yield* Effect.logDebug(`Cancelling fiber for itemId ${itemId}`);
				...
				getDownloadFiber: Effect.fn('DownloadManager.getDownloadFiber')(function* (itemId) {
					...
					yield* Effect.logDebug(`Retrieved fiber for itemId ${itemId}`);
					return fiber;
				}),
